...
<div class="flex items-center gap-3 mb-8">
    ...
    <div>
        <h1 class="text-xl font-extrabold text-white tracking-tight">Nouvelle tâche</h1>
        <p class="text-white/30 text-xs mt-0.5">Remplissez les informations ci-dessous</p>
    </div>
</div>
...
<div >
    <div class="relative glass rounded-2xl border border-white/8 overflow-hidden">
        ...
        <div class="absolute top-0 left-0 right-0 h-[2px]
                    bg-gradient-to-r from-[#FF0B55] via-pink-400 to-transparent"></div>

        {{-- Glow décoratif --}}
        <div class="absolute -top-10 -right-10 w-40 h-40 rounded-full
                    bg-[#FF0B55]/10 blur-3xl pointer-events-none"></div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text"
                           ...
                           class="w-full px-4 py-3 rounded-xl
                                  ...
                                  focus:outline-none focus:border-[#FF0B55]/50 focus:bg-white/7
                ...
                <div class="space-y-2">
                    ...
                    <textarea id="description"
                              ...
                              class="w-full px-4 py-3 rounded-xl resize-none
                                     ...
                                     focus:outline-none focus:border-[#FF0B55]/50 focus:bg-white/7
                                     transition-all duration-200
                                     @error('description') border-red-500/50 bg-red-500/5 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-xs flex items-center gap-1 mt-1">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        ...
                        <label class="status-option cursor-pointer">
                            <input type="radio" name="status" value="todo"
                                   class="sr-only" {{ old('status', 'todo') === 'todo' ? 'checked' : '' }}>
                        ...
                        <label class="status-option cursor-pointer">
                            <input type="radio" name="status" value="in_progress"
                                   class="sr-only" {{ old('status') === 'in_progress' ? 'checked' : '' }}>
                            <div class="status-card flex flex-col items-center gap-2 px-3 py-4 rounded-xl
                                        border border-white/10 bg-white/5
                        ...
                        <label class="status-option cursor-pointer">
                            <input type="radio" name="status" value="completed"
                                   class="sr-only" {{ old('status') === 'completed' ? 'checked' : '' }}>
                            <div class="status-card flex flex-col items-center gap-2 px-3 py-4 rounded-xl
                                        border border-white/10 bg-white/5
...
<style>
    ...
    .status-option input[type="radio"]:checked + .status-card {
        border-color: rgba(255,11,85,0.45) !important;
        background: rgba(255,11,85,0.12) !important;
        box-shadow: 0 0 0 1px rgba(255,11,85,0.3);
    }
    .status-option input[type="radio"]:checked + .status-card span {
        color: white !important;
    }

    /* Textarea & input autofill fix */
...
<script>
    ...
    // ── Status card : couleur selon choix ───────────────────────
